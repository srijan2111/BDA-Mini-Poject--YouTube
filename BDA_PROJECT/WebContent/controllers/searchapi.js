...
var API_ACCESS_KEY = 'AIzaSyB0FUKCy_9Gvv908m6gRCwANXVndVo-8Zo';
...
function playlistInfoRecursive(playlistId,pageToken,num,cback){
	...
	key: API_ACCESS_KEY
	  ...
	  var videoIDRequest = youtube.videos.list({
		...
		fields : 'items(id,snippet(categoryId,defaultLanguage,publishedAt),contentDetails(duration),statistics(viewCount,commentCount,likeCount,dislikeCount))',
			...
			var videoRequestVideoId = videodata.id;
			  for (var i = 0; i < videoArr.length; i++) {
				if (videoArr[i].videoId === videoRequestVideoId) {
				  ...
				  videoArr[i].defaultLanguage = videodata.snippet.defaultLanguage;
				  videoArr[i].publishedAt = videodata.snippet.publishedAt;
			...
			var videocategRequestcategoryId = categdata.id;
			  for (var i = 0; i < videoArr.length; i++) {
				if (videoArr[i].categoryId === 	videocategRequestcategoryId) {
					...
					var fields = ['title','videoId','categoryId' , 'category', 'duration', 'viewCount' , 'commentCount', 'channelId','likeCount','dislikeCount','defaultLanguage','publishedAt' ];
